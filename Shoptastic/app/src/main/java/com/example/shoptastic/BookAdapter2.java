...
public class BookAdapter2 extends RecyclerView.Adapter<BookAdapter2.ViewHolder> {
    ...
    public BookAdapter2(Context context, List<Model> modelList) {
        this.context = context;
        this.modelList = modelList;
    }

    @Override
    public BookAdapter2.ViewHolder onCreateViewHolder(ViewGroup parent, int i) {

        View view = LayoutInflater.from(context).inflate(R.layout.listitem, parent, false);
        return new BookAdapter2.ViewHolder(view);

    }

    @Override
    public void onBindViewHolder(BookAdapter2.ViewHolder holder, int position) {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            if (position == 0) {
                Intent intent = new Intent(context, Book5Activity.class);
                context.startActivity(intent);
            }

            if (position == 1) {
                Intent intent = new Intent(context, Book6Activity.class);
                context.startActivity(intent);
            }

            if (position == 2) {
                Intent intent = new Intent(context, Book7Activity.class);
                context.startActivity(intent);
            }

            if (position == 3) {
                Intent intent = new Intent(context, Book8Activity.class);
                context.startActivity(intent);
            }

            if (position == 4) {
                Intent intent = new Intent(context, Book9Activity.class);
                context.startActivity(intent);
            }
