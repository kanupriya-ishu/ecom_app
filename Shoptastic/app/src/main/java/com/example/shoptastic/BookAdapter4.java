...
public class BookAdapter4 extends RecyclerView.Adapter<BookAdapter4.ViewHolder> {
    ...
    public BookAdapter4(Context context, List<Model> modelList) {
        this.context = context;
        this.modelList = modelList;
    }

    @Override
    public BookAdapter4.ViewHolder onCreateViewHolder(ViewGroup parent, int i) {

        View view = LayoutInflater.from(context).inflate(R.layout.listitem, parent, false);
        return new BookAdapter4.ViewHolder(view);

    }

    @Override
    public void onBindViewHolder(BookAdapter4.ViewHolder holder, int position) {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            if (position == 0) {
                Intent intent = new Intent(context, Book14Activity.class);
                context.startActivity(intent);
            }

            if (position == 1) {
                Intent intent = new Intent(context, Book15Activity.class);
                context.startActivity(intent);
            }

            if (position == 2) {
                Intent intent = new Intent(context, Book16Activity.class);
                context.startActivity(intent);
            }

            if (position == 3) {
                    Intent intent = new Intent(context, Book17Activity.class);
                context.startActivity(intent);
            }

            if (position == 4) {
                Intent intent = new Intent(context, Book18Activity.class);
                context.startActivity(intent);
            }
